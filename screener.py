...
def score_row(row, sec_hits, news_hits, float_info) -> tuple:
    ...
    if float_shares:
        if float_shares < 5_000_000:
            score += 15
            reasons.append(f"Very low float ({float_shares/1e6:.1f}M sh)")
        elif float_shares < 20_000_000:
            score += 8
            reasons.append(f"Low float ({float_shares/1e6:.1f}M sh)")

    mcap = row.get("market_cap")
    if score > 0 and pd.notna(mcap) and mcap and mcap < 50_000_000:
        score += 5
        reasons.append("Micro-cap (<$50M)")

    return round(score, 1), reasons


def run(max_price=MAX_PRICE, max_market_cap=MAX_MARKET_CAP, out_dir="data"):
    ...
    candidates.to_csv(f"{out_dir}/results.csv", index=False)
    ...
    print(f"Done. Wrote {out_dir}/results.json and {out_dir}/results.csv")
