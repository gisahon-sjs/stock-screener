...
if errorlevel 1 (
    ...
    python "%~dp0notify_telegram.py" --prefix results >> "%~dp0pull_latest.log" 2>&1
    python "%~dp0notify_webhook.py" --prefix results >> "%~dp0pull_latest.log" 2>&1
    python "%~dp0notify_toast.py" --prefix results >> "%~dp0pull_latest.log" 2>&1
)
